...
class Trader:
    ...
    def convert(self, position: int) -> int:
        conversions = -position
        return conversions

    def macarons_implied_bid_ask(self,observation: ConversionObservation) -> (float, float):

        macarons_implied_bid = observation.bidPrice - observation.exportTariff - observation.transportFees - 0.1
        macarons_implied_ask = observation.askPrice + observation.importTariff + observation.transportFees

        return macarons_implied_bid, macarons_implied_ask
    
    def macarons_take_strategy(
        self,
        order_depth: OrderDepth,
    ...
    def run(self, state: TradingState):
        ...
        #########################################################################################################################################################################
         ...
         ################################################################################################################################################################################################
        ######################################   TRADE OPTIONS - GAMMA & DELTA HEDGING   ########################################################################################################################
        ################################################################################################################################################################################################
        ...
        higher_prod = Product.VOLCANIC_ROCK_VOUCHER_10500
        lower_prod = Product.VOLCANIC_ROCK_VOUCHER_10000
        ...
        ################################################################################################################################################################################################
        ########################################   TRADE BASKETS - SPREAD ARBITRAGE   ########################################################################################################################
        ################################################################################################################################################################################################
        ...
        if Product.MAGNIFICENT_MACARONS in self.params and Product.MAGNIFICENT_MACARONS in state.order_depths:
            ...
            conversions = self.convert(macarons_position)

            macarons_position = 0

            macarons_take_orders, buy_order_volume, sell_order_volume = (
                self.macarons_take_strategy(
            ...
            result[Product.MAGNIFICENT_MACARONS] = macarons_take_orders
